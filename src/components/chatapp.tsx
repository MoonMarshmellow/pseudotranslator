...
import { requestClientAllowed } from '@/lib/requestClientAllowed';
import useClientRequest from '@/lib/useClientRequest';
...
export default function ChatApp() {
    ...
    const {checkAllowed} = useClientRequest()
    const [user] = useAuthState(auth)
    const [noUses, setNoUses] = useState(false);
    const fingerprint = useFingerprint()
    const {messages, input, handleInputChange, handleSubmit, data} = useChat({body: {allowed: allowed}})
    ...
    return(
        ...
        <div className="text-white font-sans border-green-400 border-0">
            ...
            <div className='flex justify-center'>
                ...
                <button className='bg-white' onClick={onSubmit}>Ass</button>
                </div>
            </div>
